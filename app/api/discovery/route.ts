...
import {rankCandidatesWithAuraV1} from "../../../lib/server/match/recommendation";
...
export async function POST(request:Request){
  try{
    ...
    const rankingSnapshotAt=new Date().toISOString();
    const rankedData=await rankCandidatesWithAuraV1({viewerId:user.id,candidates:(data??[]) as Array<Record<string,unknown>&{id:string}>,snapshotAt:rankingSnapshotAt});
