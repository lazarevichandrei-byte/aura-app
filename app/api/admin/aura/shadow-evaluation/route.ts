...
export async function POST(request:Request){try{
 const body=await request.json().catch(()=>null);const authorization=authorizeAuraAdmin(body?.initData);if(!authorization.ok)return NextResponse.json({ok:false,error:"NOT_FOUND"},{status:404});
 const {data:outcomes,error:outcomeError}=await supabaseAdmin.from("aura_match_outcomes").select("viewer_user_id,candidate_user_id,window_type,score_snapshot_id,outcomes").eq("is_window_complete",true).in("window_type",WINDOWS).not("score_snapshot_id","is",null).order("evaluated_at",{ascending:false}).limit(3000);if(outcomeError)throw outcomeError;
 ...
 if(scoreIds.length){
  const {data:activeScores,error:activeError}=await supabaseAdmin.from("aura_match_score_snapshots").select("id,viewer_user_id,candidate_user_id,score_version,snapshot_at,total_score").in("id",scoreIds).eq("score_version",2);if(activeError)throw activeError;
  const active=(activeScores??[]) as ScoreRow[],activeById=new Map(active.map(row=>[row.id,row])),snapshotTimes=[...new Set(active.map(row=>row.snapshot_at))];let shadows:ScoreRow[]=[];
  if(snapshotTimes.length){const {data,error}=await supabaseAdmin.from("aura_match_score_snapshots").select("id,viewer_user_id,candidate_user_id,score_version,snapshot_at,total_score").eq("score_version",3).in("snapshot_at",snapshotTimes).limit(5000);if(error)throw error;shadows=(data??[]) as ScoreRow[];}
