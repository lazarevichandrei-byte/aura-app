...
import {loadAuraTrainingDatasetV1} from "../../../../../lib/server/match/learning/training-dataset-v1";
...
export async function POST(request:Request){
 try{
  ...
  const dataset=await loadAuraTrainingDatasetV1("24h",2000);
  const evaluation=evaluateAuraLearningCandidateOfflineV1(dataset.examples);
  const [{count:v2},{count:v3},{count:candidate},{data:latest}]=await Promise.all([
  ...
  return NextResponse.json({ok:true,generatedAt:new Date().toISOString(),training:{count:dataset.examples.length,required:100,remaining:Math.max(0,100-dataset.examples.length)},scores:{v2:v2??0,v3:v3??0,candidate:candidate??0},evaluation,latest:latest??[],policy:{productionRanking:"V2",candidate:"SHADOW_ONLY",automaticPromotion:false}});
