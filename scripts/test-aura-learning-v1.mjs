...
const training=await load("training-example-v1.js");const inference=await load("inference-input-v1.js");const candidate=await load("candidate-v1.js");const offline=await load("offline-evaluation-v1.js");const runtimeModule=await load("candidate-shadow-runtime-v1.js");
...
const smallRuntime=runtimeModule.createAuraCandidateShadowRuntimeV1(examples.slice(0,20));const smallResult=runtimeModule.runAuraCandidateShadowV1(smallRuntime,input);assert.equal(smallRuntime.enabled,false);assert.equal(smallResult.executed,false);
const runtime=runtimeModule.createAuraCandidateShadowRuntimeV1(examples);const before=JSON.stringify(input);runtimeModule.runAuraCandidateShadowV1(runtime,input);assert.equal("label" in input,false);assert.equal(JSON.stringify(input),before);
