...
export async function rankCandidatesWithAuraV1<T extends RankableCandidate>({viewerId,candidates,snapshotAt,mode=auraRankingMode(),dependencies}:{viewerId:string;candidates:readonly T[];snapshotAt:string;mode?:AuraRankingMode;dependencies?:AuraRankingDependencies}):Promise<T[]>{
  ...
  try{
    ...
    console.warn("AURA_RANKING_FALLBACK",{candidateCount:candidates.length,latencyBucket:rankingDiagnosticsV1(candidates,candidates,0,Math.min(candidates.length,AURA_RANKING_V1.MAX_CANDIDATES),performance.now()-startedAt).latencyBucket,errorCode:error instanceof Error&&error.message==="AURA_RANKING_TIMEOUT"?"timeout":"scoring_failed"});
    return [...candidates];
  }
}
