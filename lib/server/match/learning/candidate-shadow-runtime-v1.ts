...
export type AuraCandidateShadowRuntimeV1={
 ...
 score:(input:AuraLearningInferenceInputV1)=>number;
};
...
export function createAuraCandidateShadowRuntimeV1(examples:AuraTrainingExampleV1[]):AuraCandidateShadowRuntimeV1{
 ...
 return {enabled,reason:enabled?"OFFLINE_GATES_PASSED":"OFFLINE_GATES_FAILED",candidate:evaluation.candidate,score:(input)=>scoreAuraLearningCandidateV1(input,evaluation.candidate)};
}

export function runAuraCandidateShadowV1(runtime:AuraCandidateShadowRuntimeV1,input:AuraLearningInferenceInputV1):AuraCandidateShadowRunResultV1{
 if(!runtime.enabled)return {executed:false,score:null,reason:runtime.reason};
 return {executed:true,score:Number(runtime.score(input).toFixed(4)),reason:"SHADOW_ONLY"};
}
