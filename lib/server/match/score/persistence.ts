...
export async function persistAuraScore(viewerUserId:string,candidateUserId:string,score:PersistableAuraScore):Promise<PersistedAuraScore>{
  ...
  if(!inserted.error)return inserted.data as PersistedAuraScore;
  ...
  return existing.data as PersistedAuraScore;
}

export const persistAuraScoreV1=(viewerUserId:string,candidateUserId:string,score:AuraScoreV1)=>persistAuraScore(viewerUserId,candidateUserId,score);
